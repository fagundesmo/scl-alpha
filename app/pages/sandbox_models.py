...
from src.config import DATA_START, RF_PARAMS, RIDGE_ALPHA, TARGET_COL, TICKERS, XGB_PARAMS, TRAIN_CUTOFF, TEST_CUTOFF
...
date_range = (
    f"{pd.to_datetime(features['Date']).min().date()} → "
    f"{pd.to_datetime(features['Date']).max().date()}"
)
st.caption(
    f"Feature matrix: {len(features):,} rows × {features.shape[1]} columns | "
...
# -----------------------------------------------------------------------
with st.expander("Split settings", expanded=True):
    use_year_split = st.toggle(
        "Use calendar-year split (recommended for time series)",
        value=True,
        help=(
            "Train: 2018–2024 | Val: 2025 | Test: 2026  \n"
            "Keeps the most recent year as true out-of-sample data, "
            "which is standard practice in quantitative finance."
        ),
    )

    if use_year_split:
        st.info(
            f"**Train:** {DATA_START} → 2024-12-31  |  "
            f"**Val:** 2025  |  "
            f"**Test:** 2026 (out-of-sample)",
            icon="📅",
        )
        val_frac = test_frac = 0.20  # not used when use_year_split=True
    else:
        c1, c2 = st.columns(2)
        val_frac = c1.slider(
            "Validation fraction",
            min_value=0.10, max_value=0.35, value=0.20, step=0.05,
...
with st.expander("Model parameters", expanded=True):
    ...
    ridge_alpha = st.slider(
        "Alpha (L2 strength)",
        min_value=0.001, max_value=100.0, value=float(RIDGE_ALPHA), step=0.1,
        help="Higher alpha → stronger regularization → more conservative predictions.",
    )

    st.markdown("#### Random Forest")
    rf1, rf2, rf3 = st.columns(3)
    rf_n_estimators = rf1.slider("Trees", min_value=50, max_value=800,
                                  value=int(RF_PARAMS.get("n_estimators", 100)), step=10)
    rf_max_depth    = rf2.slider("Max depth", min_value=2, max_value=20,
                                  value=int(RF_PARAMS.get("max_depth", 6)),
                                  help="Higher → more complex model, risk of overfitting.")
    rf_min_leaf     = rf3.slider("Min leaf size", min_value=1, max_value=50,
                                  value=int(RF_PARAMS.get("min_samples_leaf", 20)),
                                  help="Higher → smoother predictions.")

    st.markdown("#### XGBoost")
    x1, x2, x3, x4 = st.columns(4)
    xgb_n_estimators  = x1.slider("Trees", min_value=50, max_value=1000,
                                    value=int(XGB_PARAMS.get("n_estimators", 200)), step=25)
    xgb_max_depth     = x2.slider("Max depth", min_value=2, max_value=10,
                                    value=int(XGB_PARAMS.get("max_depth", 4)))
    xgb_learning_rate = x3.number_input("Learning rate", min_value=0.01, max_value=0.5,
                                          value=float(XGB_PARAMS.get("learning_rate", 0.05)),
                                          step=0.01, format="%.2f")
    xgb_early_stopping = x4.slider("Early stopping rounds", min_value=5, max_value=100,
                                     value=int(XGB_PARAMS.get("early_stopping_rounds", 20)),
                                     help="Stop if val metric doesn't improve.")

    x5, x6, x7, x8 = st.columns(4)
    xgb_subsample  = x5.slider("Subsample", min_value=0.4, max_value=1.0,
                                 value=float(XGB_PARAMS.get("subsample", 0.8)), step=0.05)
    xgb_colsample  = x6.slider("Colsample by tree", min_value=0.4, max_value=1.0,
                                 value=float(XGB_PARAMS.get("colsample_bytree", 0.8)), step=0.05)
    xgb_reg_alpha  = x7.slider("Reg alpha (L1)", min_value=0.0, max_value=10.0,
                                 value=float(XGB_PARAMS.get("reg_alpha", 0.0)), step=0.1)
    xgb_reg_lambda = x8.slider("Reg lambda (L2)", min_value=0.0, max_value=20.0,
                                 value=float(XGB_PARAMS.get("reg_lambda", 1.0)), step=0.1)

    xgb_objective = st.selectbox(
        "Objective", options=["reg:squarederror", "reg:absoluteerror", "reg:pseudohubererror"],
...
if st.button("Run selected models", type="primary", use_container_width=True):
    ...
    if results:
        ...
        st.success(f"Completed {len(results)} model(s). Go to **Predictions** to explore signals.")
...
for model_name, res in results.items():
    ...
    row["Test dates"] = res.get("test_date_range", "")
    rows.append(row)
...
st.dataframe(
    comparison_df.style.format({
        "MAE": "{:.4f}", "RMSE": "{:.4f}",
        "IC (Spearman)": "{:.3f}", "Hit Rate": "{:.1%}",
        "Pred Sharpe": "{:.2f}",
    }),
    use_container_width=True, hide_index=True,
)
st.caption(
    "IC = Spearman rank correlation (predicted vs realized). "
    "Hit Rate = fraction with correct sign. "
    "Pred Sharpe = annualised Sharpe of sign(pred)×actual daily return strategy."
)
...
split_rows = []
for model_name, res in results.items():
    for split_name, split_metrics, date_range_key in [
        ("Train", res["train_metrics"], "train_date_range"),
        ("Val",   res["val_metrics"],   "val_date_range"),
        ("Test",  res["test_metrics"],  "test_date_range"),
    ]:
        row = {
            "Model": MODEL_OPTIONS[model_name],
            "Split": split_name,
            "Dates": res.get(date_range_key, ""),
        }
...
for tab, (model_name, res) in zip(tabs, results.items()):
    with tab:
        st.markdown(f"**Parameters:** `{res['model_params']}`")
        st.caption(
            f"Train: {res['train_size']:,} rows ({res.get('train_date_range','')})  |  "
            f"Val: {res['val_size']:,} rows ({res.get('val_date_range','')})  |  "
            f"Test: {res['test_size']:,} rows ({res.get('test_date_range','')})"
        )

        # ---- Ridge: signed coefficient table ----
        coef_table = res.get("coef_table")
        if isinstance(coef_table, pd.DataFrame) and not coef_table.empty:
            st.markdown("#### Ridge Coefficients (signed)")
            st.caption(
                "Positive coefficient → feature predicts higher next-day return. "
            ...
            fig = px.bar(
                ...
                title="Ridge Coefficients (post-scaling, top 30 by |coef|)",
            )
            fig.update_layout(
                yaxis={"categoryorder": "total ascending"},
                height=600, margin=dict(l=10, r=10, t=40, b=20),
            ...
            with st.expander("Full coefficient table"):
                ...
                st.download_button(
                    ...
                    file_name=f"ridge_coefficients.csv",
                    mime="text/csv",
                    key=f"dl_coef_{model_name}",
                )

        # ---- RF / XGBoost: feature importance ----
        imp = res.get("feature_importance")
        if isinstance(imp, pd.DataFrame) and not imp.empty and coef_table is None:
            ...
            fig.update_layout(
                yaxis={"categoryorder": "total ascending"},
                height=500, margin=dict(l=10, r=10, t=40, b=20),
            )
            st.plotly_chart(fig, use_container_width=True)

            with st.expander("Full importance table"):
                st.dataframe(
                    imp.style.format({"importance": "{:.6f}"}),
                    use_container_width=True, hide_index=True,
                )
                st.download_button(
                    ...
                    mime="text/csv",
                    key=f"dl_imp_{model_name}",
                )

        # ---- Test predictions scatter ----
        preds = res.get("test_predictions")
        if isinstance(preds, pd.DataFrame) and not preds.empty:
            st.markdown("#### Predicted vs Actual — Test Set (2026)")
            fig = px.scatter(
                preds, x="y_true", y="y_pred", color="Ticker", opacity=0.4,
            ...
            fig = px.histogram(
                ...
                color="Ticker", barmode="overlay", opacity=0.6,
            )
            fig.update_layout(height=350)
            st.plotly_chart(fig, use_container_width=True)

            with st.expander("Test predictions table"):
                st.dataframe(
                    preds.style.format({
                        "y_true": "{:.4f}", "y_pred": "{:.4f}", "residual": "{:.4f}",
                    }),
                    use_container_width=True, hide_index=True,
                )
        ...
        # ---- Download ALL predictions (train+val+test) ----
        all_preds = res.get("all_predictions")
        if isinstance(all_preds, pd.DataFrame) and not all_preds.empty:
            st.download_button(
                label="Download ALL predictions CSV (train + val + test)",
                data=all_preds.to_csv(index=False).encode("utf-8"),
                file_name=f"{model_name}_all_predictions.csv",
                mime="text/csv",
                key=f"dl_all_{model_name}",
            )
