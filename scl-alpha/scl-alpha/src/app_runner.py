...
def _trainable_data(df: pd.DataFrame, tickers: list[str]) -> tuple[pd.DataFrame, list[str]]:
    feature_cols = [c for c in FEATURE_COLUMNS if c in df.columns]
    tradable = df[df.index.get_level_values("ticker").isin(tickers)].copy()
    trainable = tradable.dropna(subset=feature_cols + ["target_ret_5d_fwd"])
    if trainable.empty:
        raise ValueError("No trainable rows found after dropping NaNs from features/target.")
    return trainable, feature_cols
...
def run_single_model(
    ...
    trainable, feature_cols = _trainable_data(feature_df, tickers)
    trainable = trainable.sort_index()
    ...
    y_pred = predict(model_eval, X_test)
    ...
    latest = _latest_snapshot(
        feature_df[feature_df.index.get_level_values("ticker").isin(tickers)],
        feature_cols,
    )
    latest_pred = latest.reset_index()[["date", "ticker"]].copy()
    latest_pred["predicted_ret"] = predict(model_full, latest[feature_cols])
