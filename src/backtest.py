...
def _period_return(
    ...
    """
    exec_date = signal_date + timedelta(days=3)  # Fri -> Mon
    dates = df.index.get_level_values("date").unique().sort_values()
    ...
    exit_date = future_dates[FORWARD_RETURN_DAYS - 1]
...
def run_backtest(
    ...
    df : pd.DataFrame
        ...
        Must contain training features and 'target_ret_5d_fwd'.
    model_name : str
    ...
    feature_columns : list[str] | None
        Optional explicit feature list to use for train/predict.

    Returns
    -------
    pd.DataFrame
        One row per rebalance date with columns:
        [date, holdings, gross_return_pct, net_return_pct,
         cumulative_return, turnover_frac, cost_pct].
    """
    ...
    print(
        f"[backtest] {len(rebal_dates)} rebalance dates from "
    ...
    for i, sig_date in enumerate(rebal_dates):
        if model is None or i % retrain_every == 0:
            ...
            print(
                f"  [backtest] Retrained {model_name} at {sig_date.date()} "
        ...
        preds = predict(model, sig_data, feature_cols=feature_cols)
        sig_data = sig_data.copy()
        sig_data["predicted_ret"] = preds

        ranked = sig_data.reset_index().sort_values("predicted_ret", ascending=False)
        longs = ranked[ranked["predicted_ret"] > 0].head(top_k)["ticker"].tolist()

        gross_ret = _period_return(df, sig_date, longs) if longs else 0.0

        changed = set(longs) ^ set(prev_holdings)
        trade_frac = len(changed) / max(top_k, 1)
        cost = trade_frac * (cost_bps + slippage_bps) / 100.0
    ...
    print(
        f"[backtest] Done. {len(res_df)} periods, "
...
def buy_and_hold_baseline(
    ...
    Returns a DataFrame with column ['cumulative_return'].
    """
